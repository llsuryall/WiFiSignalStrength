...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void updateAPList(){
        if(this.wifiDetails.scanResults==null){
            return;
        }
        LayoutInflater layoutInflater = getLayoutInflater();
        this.apList.removeAllViews();
        if(this.wifiDetails.scanResults.size()==0){
            layoutInflater.inflate(R.layout.no_ap_available,apList,true);
        }else{
            HorizontalScrollView cur_hsv;
            TextView cur_tv;
            for(ScanResult scanResult:this.wifiDetails.scanResults){
    ...
    public boolean scanWifi(){
       boolean scanned = this.wifiManager.startScan();
       this.wifiDetails.scanResults=this.wifiManager.getScanResults();
       return scanned;
    }
    public void updateLastScanTime(){
        if(this.wifiDetails.lastScanned !=null) {
            ((TextView) findViewById(R.id.lastUpdatedTime)).setText(
                    getString(
                            R.string.last_updated,
                            SimpleDateFormat.getTimeInstance().format(this.wifiDetails.lastScanned)
                    )
            );
        }
    }
    public void onClick(View view) {
        ...
        //Start scanning
        if(this.scanWifi()) {
            this.wifiDetails.lastScanned =Calendar.getInstance().getTime();
            this.updateLastScanTime();
            this.updateAPList();
        }
    }
